  ...
  return (
    <div className="my-3 flex flex-row rounded-lg border-2 border-slate-300 bg-slate-50 py-5 px-5">
      ...
      <div className="flex flex-col">
        <div className={`flex flex-col ${show.image_url ? "pl-10" : ""} pr-10`}>
        ...
        >
          ...
          <Form method="post">
            <input type="hidden" name="showId" value={show.maze_id} />
            <button
              type="submit"
              className="mt-4 rounded bg-slate-600 py-2 px-4 hover:bg-slate-500 active:bg-slate-500"
            >
              {!transition.submission && (
                <span className="text-white">Add Show</span>
              )}
              {!!transition.submission && <Spinner />}
            </button>
          </Form>
        </div>
      </div>
